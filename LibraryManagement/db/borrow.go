...
func (r *BookRequestRepo) GetUserHistory(uId *int, search string, reqStatus string) ([]*UserBookHistory, error) {

	query := GetQueryBuilder().Select("rq.request_id", "rq.bookid", "b.title AS book_title", "rq.read_page", "b.total_page", "rq.request_status", "rq.issued_at", "rq.created_at").
		From("book_request rq").
		Join("book b ON rq.bookid = b.id").
		Where(sq.Eq{"rq.readerid": *uId})
	if search != "" {
		// Construct a LIKE clause with case insensitivity
		query = query.Where(sq.Expr("LOWER(b.title) LIKE ?", "%"+strings.ToLower(search)+"%"))
	}
	if reqStatus != "" {
		query = query.Where(sq.Eq{"rq.request_status": reqStatus})
	}
...
func (r *BookRequestRepo) UpdateBookRequestStatus(reqId int) error {
	...
	if err != nil {
		slog.Error(
			"Failed to create update query for book_request table to update book_status",
			logger.Extra(map[string]any{
				"error": err.Error(),
				"query": updateQry,
				"args":  args,
	...
	if err != nil {
		slog.Error(
			"Failed to update of book_request table's book_status",
			logger.Extra(map[string]any{
				"error": err.Error(),
				"query": updateQry,
				"args":  args,
...
func (r *BookRequestRepo) ValidateUserBorrowRequest(req *BookRequest) error {

	fmt.Println(req)
	qry, args, err := GetQueryBuilder().Select("COUNT(*)").
		From(r.Table).
		Where(sq.Eq{"readerid": req.ReaderId}).Where(sq.Eq{"bookid": req.BookId}).
		Where(sq.Or{
			sq.Eq{"request_status": "Approved"},
			sq.Eq{"request_status": "Pending"}}).ToSql()
